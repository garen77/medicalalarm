...
class CameraManager: NSObject, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIPopoverPresentationControllerDelegate {
    ...
    var completionWithCameraType = false
    var completionHandler : ((_ image: UIImage) -> ())!
    var completionHandlerCameraType : ((_ image: UIImage, _ cameraType: UIImagePickerControllerCameraDevice) -> ())!
    ...
    func newImageLibrary(controller: UIViewController, sourceIfPad: UIView?, editing: Bool, completionHandler : @escaping (_ image: UIImage) -> ()) {
        ...
        self.completionWithCameraType = false
    ...
    //scatta una nuova foto
    func newImageShootCameraType(controller: UIViewController, sourceIfPad: UIView?, editing: Bool, overlay: UIImageView?, completionHandler : @escaping (_ image: UIImage, _ cameraType: UIImagePickerControllerCameraDevice) -> ()) {
        
        self.completionHandlerCameraType = completionHandler
        self.completionWithCameraType = true
        let picker = UIImagePickerController()
        picker.delegate = self
        picker.sourceType = UIImagePickerControllerSourceType.camera;
        let arra : [AnyObject] = [kUTTypeImage]
        ...
        if mediaType == kUTTypeImage {
            ...
            if var test = imageEdited {
                if picker.cameraDevice == .front {
                    test = UIImage(cgImage: test.cgImage!, scale: test.scale, orientation: .leftMirrored)
                }
                if self.completionWithCameraType {
                    self.completionHandlerCameraType(test,picker.cameraDevice)
                } else {
                ...
                if self.completionWithCameraType {
                    self.completionHandlerCameraType(imageOriginal,picker.cameraDevice)
                } else {
